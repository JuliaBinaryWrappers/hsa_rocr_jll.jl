...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import hsa_rocr_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("hsa_rocr_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "hsa_rocr_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"hsa_rocr", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "hsa_rocr_jll", "src", "hsa_rocr_jll.jl"))
        @info("hsa_rocr_ll dev'ed out to /depot/dev/hsa_rocr_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("dd59ff1a-a01a-568d-8b29-0669330f116a"))
platforms = [Pkg.Artifacts.unpack_platform(e, "hsa_rocr", artifacts_toml) for e in artifacts["hsa_rocr"]]
...
if best_platform === nothing
    @debug("Unable to load hsa_rocr; unsupported platform $(triplet(platform_key_abi()))")
else
